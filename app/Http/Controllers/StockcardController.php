...
class StockcardController extends Controller {
    ...
    public function stockcards(Request $request) {
        ...
        $stockcards = DB::table("stock_card")
                ...
                ->selectRaw("cf_company.descs as company_descs, 
                        CONVERT(varchar(20),stock_card.audit_date,100) as audit_date, 
                ...
                ->orderBy('stock_card.audit_date', 'desc')
    ...
    public function get_issuance(Request $request) {
        ...
        $issuance = DB::table("stock_card")
                ...
                ->where('stock_card.item_category_cd', $item_category_cd)
                ->where('stock_card.trx_type', $trx_type)
                ->where('stock_card.status', $status)
                ->where('stock_card.inout_mode', 'O')
                ->selectRaw("cf_company.descs as company_descs, 
                        CONVERT(varchar(12),stock_card.audit_date,102) as audit_date, 
                        stock_card.audit_user, 
                        cf_department.descs as department, 
    ...
    public function user_activity(Request $request) {
        ...
        $user_modified = DB::table('stock_card')
                ->where('stock_card.audit_user', $username)
                ->whereMonth('stock_card.audit_date', '=', $month)
                ->whereYear('stock_card.audit_date', '=', $year)
                ->count();

        $user_activity = new \stdClass();
        $user_activity->created = $user_created;
        $user_activity->approved = $user_approved;
        $user_activity->modified = $user_modified;
